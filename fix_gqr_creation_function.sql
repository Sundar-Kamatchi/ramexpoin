-- Fix the GQR creation RPC function to match current database schema
-- This function creates a GQR entry and updates the pre_gr_entry
...
BEGIN
  ...
  RETURN QUERY SELECT v_gqr_id, v_gr_no, TRUE, 'GQR created successfully'::TEXT;
...
-- Also create a simpler version for testing if needed
CREATE OR REPLACE FUNCTION create_gqr_and_update_pre_gr_simple(
  p_pre_gr_id INTEGER,
  p_export_quality_weight NUMERIC,
  p_rot_weight NUMERIC,
  p_doubles_weight NUMERIC,
...
DECLARE
  ...
  v_created_at TIMESTAMPTZ := NOW();
BEGIN
  -- Get the gr_no from the pre_gr_entry
  SELECT gr_no INTO v_pre_gr_gr_no
  FROM pre_gr_entry
  WHERE id = p_pre_gr_id;
  ...
  -- Insert the GQR entry (without bag counts)
  INSERT INTO gqr_entry (
    pre_gr_entry_id,
    gr_no,
    export_quality_weight,
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION create_gqr_and_update_pre_gr_simple(
