-- Fix the function to use the correct column name for GQR number
...
BEGIN
    ...
    -- Get the next GQR number - try both possible column names
    BEGIN
        SELECT COALESCE(MAX(gqr_no), 0) + 1 INTO v_gqr_no
        FROM gqr_entry;
    EXCEPTION
        WHEN undefined_column THEN
            -- If gqr_no doesn't exist, try gr_no
            SELECT COALESCE(MAX(gr_no), 0) + 1 INTO v_gqr_no
            FROM gqr_entry;
    END;
    
    -- Insert GQR entry - try both possible column names
    BEGIN
        INSERT INTO gqr_entry (
            pre_gr_id,
            gqr_no,  -- Try gqr_no first
