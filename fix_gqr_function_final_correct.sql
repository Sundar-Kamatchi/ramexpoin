-- Final fix for create_gqr_and_update_pre_gr function
-- Using the correct column name 'pre_gr_id' that actually exists in gqr_entry table
...
BEGIN
    ...
    INSERT INTO gqr_entry (
        pre_gr_id,  -- CORRECT: This column actually exists
        gr_no,
...
-- First, let's check what Pre-GR entries exist and their status
SELECT 
    id,
    gr_no,
    is_gqr_created,
...
-- Test the function (replace 1 with a valid Pre-GR ID from the query above)
SELECT public.create_gqr_and_update_pre_gr(
    1, -- p_pre_gr_id (use a valid ID from the query above)
